...
public class SnapshotGeneratorChain {
    ...
    public <T extends DatabaseObject> T snapshot(T example, DatabaseSnapshot snapshot)
        ...
        SnapshotGenerator next = getNextValidGenerator(); // 依次获取snapshotGenerators 这个迭代器中的SnapshotGenerator

        if (next == null) {
            return null;
        }

        T obj = next.snapshot(example, snapshot, this); //不断调用SnapshotGenerator 中获取数据库对象的方法，并填充到example中
