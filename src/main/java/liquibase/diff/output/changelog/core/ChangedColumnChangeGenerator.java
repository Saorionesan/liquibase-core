...
/**
 * 1. 可以看到ChangeGenerator 分为三个模块：改变（源数据库和目标数据库不一样的 需要update）、缺失（源数据库源有，目标数据源没有（需要添加））、需要删除（源数据库源没有，目标数据源有（需要删除））下三种
 * 2. 如果要手动添加其他模块的比较（存储过程、触发器等）那么建议这三种都要添加
 * 3. 最后在生成changeSet集合时需要调用这三个部分的ChangeGenerator
 */
